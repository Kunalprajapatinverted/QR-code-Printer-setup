...
def connect_to_database():
    try:
        mydb = mysql.connector.connect(
            host="testdb.cbk6wukoo9oo.ap-south-1.rds.amazonaws.com", # Replace with your actual host, e.g., "localhost" or an IP address
            user="kunalprajapat",  # Your MySQL username
            password="Kunal_inverted",      # Your MySQL password
            database="BMS_barcode" # The name of your database
        )
        return mydb
...
def upload_barcode_to_db(barcode, product_description):
    """Upload the generated barcode to the MySQL database."""
    # Connect to the MySQL database
    mydb = connect_to_database()
    if mydb is None:
        print("Failed to connect to the database.")
    ...
    try:
        ...
        # Insert the generated barcode into the barcode table with the IST timestamp and other fields
        query = """INSERT INTO barcode (barcode_value, created_at, product_name, product_description)
                   VALUES (%s, %s, %s, %s)"""
        cursor.execute(query, (barcode, ist_time_str, product_name, product_description))
        mydb.commit()  # Commit the transaction to save changes
        print(f"Barcode {barcode} has been uploaded to the database.")
    except mysql.connector.Error as err:
        print(f"Error uploading barcode to the database: {err}")
    finally:
        cursor.close()
        mydb.close()

def check_and_generate_barcode(bms_spec):
    """Check if a barcode for the current date exists and generate the next barcode."""
    # Connect to the MySQL database
    mydb = connect_to_database()
    if mydb is None:
        return None, None
    ...
    # Query to check the last barcode for the current date
    query = "SELECT barcode_value FROM barcode WHERE DATE(created_at) = %s AND barcode_value LIKE %s ORDER BY barcode_value DESC LIMIT 1"
    ...
    if result:
        # Barcode for today exists, extract the serial number from the last barcode
        last_barcode = result[0]
        # Extract the serial number from the last barcode (assuming it's always 3 digits at the end)
        last_serial_number = int(last_barcode[-3:])
        new_serial_number = last_serial_number + 1
        return new_serial_number, last_barcode
    else:
        # No barcode exists for today, start from 001
        return 1, None
...
def main():
    """Main function to run the barcode generation process."""
    while True:
            ...
            try:
                ...
                if user_choice == 0:
                    print("Exiting barcode generation process.")
                    ...
                    # Check if a barcode exists for today, and get the next serial number
                    new_serial_number, last_barcode = check_and_generate_barcode(selected_spec)
                    
                    # If no barcode exists, start from 001
                    if new_serial_number == 1:
                        print(f"Starting new serial numbers for today.")
                    else:
                        print(f"Last barcode for today: {last_barcode}, generating new barcode with serial number {new_serial_number:03d}")
                    
                    # Generate the barcode with the new serial number
                    date_string = generate_date_format(new_serial_number)
                    barcode = f"{selected_spec}{date_string}"  # Concatenate the BMS spec with the formatted date
                    print(f"Generated Barcode: {barcode}")
                    
                    # Upload the barcode to the database with the product description
                    upload_barcode_to_db(barcode, product_description)
                else:
